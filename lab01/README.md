...
- o build da aplicação é gerado localmente,

- enviado ao S3,

- e consumido pela EC2 via IAM Role.
...
Passo 3. 
    3.1. Criar um bucket S3 chamado, por exemplo, amazn-static-react-site com ACL desabilitada (padrão moderno), acesso público bloqueado, 
    versionamento desabilitado e criptografia padrão (SSE-S3). 
...
Passo 4. Criar um IAM User (temporário) que servirá para fazer upload do site da máquina local para o bucket S3. 
    ...
    Não deve ser confundido IAM User com IAM Role, pois IAM User é usado na máquina local, no exemplo, para fazer upload dos arquivos, e IAM Role é usado na instância EC2.
    ...
    4.4. Após o usuário criado acesse-o e vá em Credenciais de Segurança (Security-Credentials) e clique em Criar chave de acesso (Create access key). Selecione o tipo Command Line Interface (CLI) e confirme a criação. Guarde em um local seguro a chave de acesso e a chave de acesso secreta, pois elas serão necessárias posteriormente.
